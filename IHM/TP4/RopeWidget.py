    ...
    def mousePressEvent(self, event):
        ...
        if(event.button() == Qt.LeftButton):
            ...
            if(temp == self.random):
                ...
                t = time() - self.timer
                self.log.append([self.num_util,"Rope",self.densite,self.taille,self.nb_repet-self.cpt,str(t).replace(".",","),1])
                self.randomTarget()
            else:
                self.error += 1
                t = time() - self.timer
                self.log.append([self.num_util,"Rope",self.densite,self.taille,self.nb_repet-self.cpt,str(t).replace(".",","),0])
